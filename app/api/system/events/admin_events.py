# app/api/system/events/admin_events.py
...
System / SuperAdmin Event moderation APIs

This module is intentionally disabled.
Will be re-enabled after system-level Event schemas are defined.
"""
...
from app.schemas.event.organizer.event_status_update import OrganizerEventStatusUpdate
...
# -------------------------------------------------------------------
@router.patch("/{event_uuid}/status", response_model=OrganizerEventResponse)
def update_event_status_admin(
    event_uuid: UUID,
    data: OrganizerEventStatusUpdate,
    db: Session = Depends(get_db),
    _admin=Depends(require_super_admin),
):
    """
